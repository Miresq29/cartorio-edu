  ...
  return (
    <div className="p-12 min-h-full bg-[#0D1B3E] animate-in fade-in space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <form onSubmit={handleCreateTenant} className="bg-[#1A2A52] border border-[#C9A84C]/30 rounded-[40px] p-12 space-y-6 shadow-2xl">
          ...
          <div className="space-y-4">
            <input 
              ...
              className="w-full bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-3xl p-5 text-white outline-none focus:border-blue-600 transition-all" required 
            />
            <input 
              type="text" value={slug} onChange={e => setSlug(e.target.value)}
              placeholder="ID do Sistema (Ex: cartorio-bh-01)"
              className="w-full bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-3xl p-5 text-blue-400 font-mono outline-none focus:border-blue-600 transition-all" required 
            />
          </div>
        ...
        <div className="bg-[#1A2A52] border border-[#C9A84C]/30 rounded-[40px] p-10 space-y-6 shadow-lg">
          <h3 className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.3em] px-2">Instâncias Ativas</h3>
          <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
            {allTenants.length === 0 && (
              <p className="text-slate-200 text-xs font-bold uppercase text-center py-10 italic">Nenhum cartório remoto carregado</p>
            )}
            {allTenants.map((t: any) => (
              <div key={t.id} className="p-5 bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl flex justify-between items-center group hover:border-blue-500/30 transition-all">
                <span className="text-white font-bold italic uppercase text-sm">{t.name}</span>
                <span className="text-[10px] font-mono text-slate-400 bg-slate-900 px-3 py-1 rounded-md">{t.id}</span>
              </div>
            ))}
          </div>
        </div>
