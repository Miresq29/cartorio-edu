        ...
        <form onSubmit={handleCreateTenant} className="bg-white border border-slate-200 rounded-[40px] p-12 space-y-6 shadow-2xl">
          ...
          <div className="space-y-4">
            <input 
              ...
              className="w-full bg-[#F8F7F2] border border-slate-200 rounded-3xl p-5 text-[#0A1628] outline-none focus:border-blue-600 transition-all" required 
            />
            <input 
              type="text" value={slug} onChange={e => setSlug(e.target.value)}
              placeholder="ID do Sistema (Ex: cartorio-bh-01)"
              className="w-full bg-[#F8F7F2] border border-slate-200 rounded-3xl p-5 text-blue-400 font-mono outline-none focus:border-blue-600 transition-all" required 
            />
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[40px] p-10 space-y-6 shadow-lg">
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
            ...
            {allTenants.map((t: any) => (
              <div key={t.id} className="p-5 bg-[#F8F7F2] border border-slate-200 rounded-2xl flex justify-between items-center group hover:border-blue-500/30 transition-all">
