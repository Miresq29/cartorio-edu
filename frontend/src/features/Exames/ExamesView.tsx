...
/* â”€â”€â”€ tipos internos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
function bloomLabel(bloom: string) {
  if (bloom === 'compreensao') return { label: 'CompreensÃ£o', color: 'blue' };
  if (bloom === 'aplicacao')   return { label: 'AplicaÃ§Ã£o',   color: 'emerald' };
  return                              { label: 'AnÃ¡lise',     color: 'purple' };
}
...
const ExamesView: React.FC = () => {
  ...
  /* resultados anteriores do usuÃ¡rio */
  ...
  /* â”€â”€ carrega fontes de conteÃºdo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  useEffect(() => {
    const unsubs: (() => void)[] = [];
    ...
    const carrega = (colecao: string, tipo: FonteConteudo['tipo'], campoConteudo: string, campoTitulo: string) => {
      const u = onSnapshot(collection(db, colecao), snap => {
        const novos = snap.docs.map(d => ({
          id: d.id,
          titulo: d.data()[campoTitulo] || d.data()['title'] || d.data()['nome'] || 'Sem tÃ­tulo',
  ...
  /* â”€â”€ carrega resultados do usuÃ¡rio â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  useEffect(() => {
    if (!user?.id) return;
    const q = query(
    ...
    return onSnapshot(q, snap => {
      ...
      // ordena client-side para evitar Ã­ndice composto no Firestore
      docs.sort((a, b) => {
  ...
  /* â”€â”€ verifica bloqueio para uma fonte â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  /* â”€â”€ verifica aprovaÃ§Ã£o anterior â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  const jaAprovado = useCallback((fonteId: string) =>
    resultados.some(r => r.fonteId === fonteId && r.aprovado),
  [resultados]);

  /* â”€â”€ gera exame â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  const handleGerarExame = async () => {
    if (!fonteEscolhida) return;

    const bloqueio = verificaBloqueio(fonteEscolhida.id);
    if (bloqueio) {
      showToast(`VocÃª estÃ¡ bloqueado por mais ${bloqueio} dia(s). Aguarde antes de tentar novamente.`, 'error');
      return;
    }

    if (!fonteEscolhida.conteudo || fonteEscolhida.conteudo.length < 50) {
      showToast('Este conteÃºdo nÃ£o possui texto suficiente para gerar um exame.', 'error');
      return;
    }
  ...
  /* â”€â”€ submete respostas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  const handleSubmeter = async () => {
    if (Object.keys(respostas).length < questoes.length) {
      showToast('Responda todas as questÃµes antes de enviar.', 'error');
      return;
    }
    ...
    try {
      ...
      showToast('NÃ£o foi possÃ­vel salvar o resultado. Recarregue a pÃ¡gina.', 'error');
    } finally {
      setSalvando(false);
    }
  };

  /* â”€â”€ certificado â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  const imprimirCertificado = () => {
    ...
    <div class="cert">
      <div class="logo">MJ Consultoria Â· Plataforma de Treinamento Corporativo</div>
      <h1>Certificado de ConclusÃ£o</h1>
      <div class="tipo">Exame de AvaliaÃ§Ã£o</div>
      ...
      <div style="display:flex;justify-content:center;gap:80px;margin-top:50px">
        <div class="assinatura">MJ Consultoria<br>CoordenaÃ§Ã£o de Treinamento</div>
      </div>
    </div></body></html>`);
    win.document.close();
    win.print();
  ...
  /* â”€â”€ progresso do exame em curso â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  const totalRespondidas = Object.keys(respostas).length;
  const percentualFeito = questoes.length > 0
    ? Math.round((totalRespondidas / questoes.length) * 100)
    : 0;

  /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• RENDER â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
    ...
    return (
      <div className="p-8 min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-6">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-500 text-sm mt-2">Elaborando questÃµes com Taxonomia de Bloom...</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">
              {questoes.length} questÃµes Â· Nota mÃ­nima {NOTA_APROVACAO}%
            </p>
          </div>
      ...
      <div className="p-8 min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-6 animate-in fade-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 text-sm max-w-sm">
            {aprovado
              ? `ParabÃ©ns! VocÃª atingiu a nota mÃ­nima de ${NOTA_APROVACAO}% neste exame.`
              : `VocÃª precisava de ${NOTA_APROVACAO}% para aprovaÃ§Ã£o. VocÃª poderÃ¡ tentar novamente em ${DIAS_BLOQUEIO} dias.`
            }
          </p>
        </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-slate-500 mt-1">
                        ...
                        {!acertou && <span className="text-emerald-400 ml-2">Â· Correta: {q.correta}</span>}
                      </p>
    ...
    <div className="p-6 md:p-8 min-h-screen bg-slate-50 space-y-6 animate-in fade-in">
      ...
      <header>
        ...
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">
          AvaliaÃ§Ãµes geradas automaticamente com Taxonomia de Bloom
        </p>
      </header>
      ...
      <div className="bg-white border border-slate-200 rounded-[20px] p-5 grid grid-cols-3 gap-4">
        {[
          { bloom: 'compreensao', label: 'CompreensÃ£o', pct: '30%', color: 'blue',    desc: 'Identificar e descrever conceitos' },
          { bloom: 'aplicacao',   label: 'AplicaÃ§Ã£o',   pct: '40%', color: 'emerald', desc: 'Utilizar e demonstrar na prÃ¡tica'  },
          { bloom: 'analise',     label: 'AnÃ¡lise',     pct: '30%', color: 'purple',  desc: 'Comparar, examinar e avaliar'      },
        ].map(b => (
      ...
      <div>
        <h3 className="text-[#0A1628] font-black uppercase text-sm mb-4">
          ...
          Escolha o conteÃºdo para o exame
        </h3>

        {loadingFontes ? (
          <div className="text-slate-500 text-sm italic">Carregando conteÃºdos...</div>
          ...
          <div className="bg-white border border-slate-200 rounded-[20px] p-8 text-center text-slate-500 text-sm italic">
            Nenhum conteÃºdo com texto suficiente encontrado.<br/>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[9px] text-slate-500 uppercase tracking-widest font-black">
                        {fonte.tipo === 'treinamento' ? 'Treinamento' :
                         ...
                         fonte.tipo === 'video' ? 'VÃ­deo' : 'Checklist'}
                      </p>
                    </div>
      ...
      {/* seletor de questÃµes + botÃ£o gerar */}
      {fonteEscolhida && (
        <div className="flex flex-col items-center gap-4 pt-2 pb-8">
          {/* seletor de quantidade */}
          <div className="flex items-center gap-2">
            <span className="text-xs font-black text-[#7a5c1e] uppercase tracking-widest">QuestÃµes:</span>
          ...
          <button onClick={handleGerarExame}
            ...
            Gerar Exame com IA
            <span className="text-blue-200 text-xs font-normal">{numQuestoes} questÃµes Â· Bloom mÃ©dio</span>
          </button>
        </div>
      )}
