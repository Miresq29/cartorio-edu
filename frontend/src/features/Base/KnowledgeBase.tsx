...
const KnowledgeBase: React.FC = () => {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 bg-[#F5F4EF] min-h-screen">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Buscar na base..."
              className="bg-slate-900 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-xs text-white outline-none focus:border-blue-500 w-56" />
          </div>
          {canManage && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {filteredDocs.map((document) => (
          <div key={document.id} className="bg-[#F5F4EF] border border-slate-200 rounded-[32px] p-8 flex flex-col justify-between shadow-lg hover:border-blue-500/30 transition-all group">
            <div>
      ...
      {viewingDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
          <div className="bg-[#F5F4EF] border border-slate-200 w-full max-w-5xl max-h-[90vh] rounded-[40px] flex flex-col overflow-hidden shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-10 bg-[#F5F4EF]">
