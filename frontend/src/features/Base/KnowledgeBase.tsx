...
const KnowledgeBase: React.FC = () => {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 bg-[#05080f] min-h-screen">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Buscar na base..."
              className="bg-slate-900 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-xs text-white outline-none focus:border-blue-500 w-56" />
          </div>
          {canManage && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {filteredDocs.map((document) => (
          <div key={document.id} className="bg-[#0a0f1d] border border-slate-800 rounded-[32px] p-8 flex flex-col justify-between shadow-lg hover:border-blue-500/30 transition-all group">
            <div>
      ...
      {viewingDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
          <div className="bg-[#0a0f1d] border border-slate-800 w-full max-w-5xl max-h-[90vh] rounded-[40px] flex flex-col overflow-hidden shadow-2xl">
            <header className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/20 flex-shrink-0">
              <div>
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => navigator.clipboard.writeText(viewingDoc.content).then(() => showToast('Copiado!', 'success'))}
                  className="text-xs text-slate-400 hover:text-white font-bold uppercase px-4 py-2 rounded-xl hover:bg-slate-800 transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-10 bg-[#05080f]">
              <pre className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap font-sans">{viewingDoc.content}</pre>
            </div>
          </div>
        </div>
      )}
