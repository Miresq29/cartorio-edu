    ...
    <div className="p-8 space-y-6 bg-[#0D1B3E] min-h-screen animate-in fade-in">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em]">Avisos e informações da empresa</p>
        </div>
        <div className="flex gap-2">
          {isGestor && (
            <>
              <button onClick={() => setModo(modo === 'mural' ? 'gestao' : 'mural')}
                className="bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input value={form.titulo} onChange={e => setForm(p => ({ ...p, titulo: e.target.value }))}
                ...
                className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-amber-500" />
            </div>
            <div className="md:col-span-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Mensagem *</label>
              <textarea value={form.corpo} onChange={e => setForm(p => ({ ...p, corpo: e.target.value }))} rows={4}
                ...
                className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-amber-500 resize-none" />
            </div>
            <div>
            ...
            <div>
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Prazo (opcional)</label>
              <input type="date" value={form.prazo} onChange={e => setForm(p => ({ ...p, prazo: e.target.value }))}
                className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-amber-500" />
            </div>
            <div>
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Link de Anexo (opcional)</label>
            ...
            <div className="flex items-center gap-3">
              ...
              <label htmlFor="fixado" className="text-sm text-slate-200 font-bold cursor-pointer">Fixar comunicado no topo</label>
            </div>
          </div>
      ...
      <div className="space-y-3">
        {ordenados.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-30">
            <i className="fa-solid fa-bell-slash text-5xl text-slate-300 mb-4"></i>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Nenhum comunicado publicado</p>
          </div>
        ) : ordenados.map(c => {
          ...
          return (
            <div key={c.id} className={`bg-white border rounded-[20px] overflow-hidden transition-all ${
              c.prioridade === 'urgente' ? 'border-red-500/40' : lido ? 'border-[#C9A84C]/30' : 'border-amber-500/30'
            }`}>
              <div className="p-5 flex items-start gap-4 cursor-pointer" onClick={() => { setExpandido(aberto ? null : c.id); marcarLido(c.id); }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <h3 className={`text-sm font-black ${lido ? 'text-slate-200' : 'text-white'} truncate`}>{c.titulo}</h3>
                ...
                <i className={`fa-solid fa-chevron-down text-slate-300 transition-transform ${aberto ? 'rotate-180' : ''}`}></i>
              </div>
              {aberto && (
                <div className="px-5 pb-5 border-t border-[#C9A84C]/30/50 pt-4 space-y-3">
                  <p className="text-sm text-slate-200 leading-relaxed whitespace-pre-wrap">{c.corpo}</p>
                  {c.anexoUrl && (
                    <a href={c.anexoUrl} target="_blank" rel="noreferrer"
                      className="inline-flex items-center gap-2 text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-2 rounded-xl font-black uppercase tracking-widest transition-all">
