            ...
            return (
              ...
              >
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${isActive ? 'bg-white/20' : 'bg-slate-800'}`}>
                  <i className={`fa-solid ${s.icon} text-xs ${isActive ? 'text-white' : 'text-slate-500'}`}></i>
                </div>
                <div className="min-w-0">
                  <p className={`text-xs font-black truncate ${isActive ? 'text-white' : 'text-slate-400'}`}>{s.title}</p>
