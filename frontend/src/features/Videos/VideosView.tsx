    ...
    <div className="p-8 space-y-6 bg-[#FBF7EE] min-h-screen animate-in fade-in">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[#5A6E8A] text-[10px] font-black uppercase tracking-[0.3em]">Biblioteca de conteúdo em vídeo</p>
        </div>
        {isGestor && (
      ...
      {showForm && isGestor && (
        <div className="bg-white border border-[#E8D5A3] rounded-[24px] p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-[9px] font-black text-[#5A6E8A] uppercase tracking-widest block mb-1">Título *</label>
              <input value={form.titulo} onChange={e => setForm(p => ({ ...p, titulo: e.target.value }))}
                placeholder="Ex: Introdução ao Registro de Imóveis"
                className="w-full bg-slate-900 border border-[#E8D5A3] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="text-[9px] font-black text-[#5A6E8A] uppercase tracking-widest block mb-1">Link do YouTube *</label>
              <input value={form.youtubeUrl} onChange={e => setForm(p => ({ ...p, youtubeUrl: e.target.value }))}
                placeholder="https://youtube.com/watch?v=..."
                className="w-full bg-slate-900 border border-[#E8D5A3] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="text-[9px] font-black text-[#5A6E8A] uppercase tracking-widest block mb-1">Categoria</label>
            ...
            <div>
              <label className="text-[9px] font-black text-[#5A6E8A] uppercase tracking-widest block mb-1">Trilha Associada (opcional)</label>
              <input value={form.trilhaTitulo} onChange={e => setForm(p => ({ ...p, trilhaTitulo: e.target.value }))}
                placeholder="Ex: Trilha de Atendimento"
                className="w-full bg-slate-900 border border-[#E8D5A3] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="text-[9px] font-black text-[#5A6E8A] uppercase tracking-widest block mb-1">Duração (minutos)</label>
              <input type="number" min={1} max={60} value={form.duracaoMin} onChange={e => setForm(p => ({ ...p, duracaoMin: +e.target.value }))}
                className="w-full bg-slate-900 border border-[#E8D5A3] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="text-[9px] font-black text-[#5A6E8A] uppercase tracking-widest block mb-1">Descrição</label>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowForm(false)} className="bg-slate-800 hover:bg-slate-700 text-[#1A2744] px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
              Cancelar
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar vídeo..."
          className="bg-white border border-[#E8D5A3] rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-red-500 w-64" />
        <button onClick={() => setFiltroCategoria('')}
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${!filtroCategoria ? 'bg-red-600 text-white' : 'bg-slate-900 text-[#8A9BB0] hover:bg-slate-800'}`}>
          Todos
        </button>
        {CATEGORIAS.map(c => (
          <button key={c.id} onClick={() => setFiltroCategoria(filtroCategoria === c.id ? '' : c.id)}
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filtroCategoria === c.id ? `bg-${c.color}-600 text-white` : 'bg-slate-900 text-[#8A9BB0] hover:bg-slate-800'}`}>
            <i className={`fa-solid ${c.icon} mr-1.5`}></i>{c.label}
          </button>
        ))}
        <span className="ml-auto text-[10px] text-[#5A6E8A] font-black uppercase tracking-widest">{videosFiltrados.length} vídeo(s)</span>
      </div>

      {/* Grid de vídeos */}
      {videosFiltrados.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 opacity-30">
          <i className="fa-brands fa-youtube text-5xl text-[#2C3E5A] mb-4"></i>
          <p className="text-[#5A6E8A] text-xs font-bold uppercase tracking-widest">Nenhum vídeo encontrado</p>
          {isGestor && <p className="text-[#2C3E5A] text-[10px] mt-1">Clique em "Adicionar Vídeo" para começar</p>}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {videosFiltrados.map(v => {
            ...
            return (
              <div key={v.id} className={`bg-white border rounded-[24px] overflow-hidden transition-all hover:scale-[1.02] group ${visto ? 'border-emerald-500/30' : 'border-[#E8D5A3]'}`}>
                {/* Thumbnail */}
                ...
                <div className="p-4 space-y-2">
                  ...
                  {v.descricao && <p className="text-[10px] text-[#5A6E8A] leading-relaxed line-clamp-2">{v.descricao}</p>}
                  <div className="flex gap-2 pt-1">
                    ...
                    {isGestor && (
                      <button onClick={() => excluirVideo(v.id)}
                        className="w-9 h-9 bg-slate-900 hover:bg-red-500/20 text-[#5A6E8A] hover:text-red-400 rounded-xl flex items-center justify-center transition-all">
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setPlayerVideo(null)}>
          <div className="w-full max-w-4xl space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setPlayerVideo(null)} className="text-[#8A9BB0] hover:text-white w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center">
            ...
            {playerVideo.descricao && (
              <p className="text-[#8A9BB0] text-sm leading-relaxed">{playerVideo.descricao}</p>
            )}
          </div>
        </div>
      )}
