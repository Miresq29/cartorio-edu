    ...
    <div className="space-y-4">
      <div className="flex justify-between items-start gap-4">
        <div>
          ...
          <p className="text-[9px] text-slate-400 uppercase tracking-widest mt-1">Gere 3 opções de roteiro e escolha a melhor para sua equipe</p>
        </div>
      ...
      <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-4">
      ...
      {(checklists.length > 0 || knowledgeDocs.length > 0) && (
        <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-4 space-y-2">
          <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Base Ativa</h4>
          {checklists.slice(0, 3).map((c, i) => (
            <div key={i} className="flex items-center gap-3 p-2 bg-slate-900/50 rounded-xl">
              <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
              <span className="text-xs text-slate-200"><span className="font-bold text-white">{c.title}</span> — {c.items?.length || 0} requisitos</span>
            </div>
          ))}
          {knowledgeDocs.slice(0, 3).map((d, i) => (
            <div key={i} className="flex items-center gap-3 p-2 bg-slate-900/50 rounded-xl">
              <div className="w-2 h-2 bg-amber-500 rounded-full"></div>
              <span className="text-xs text-slate-200"><span className="font-bold text-white">{d.fileName || d.title}</span> — documento indexado</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="h-80 overflow-y-auto p-6 space-y-4 custom-scrollbar">
          ...
          {selectedOption && messages.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              <button onClick={generateOptions} disabled={isLoading}
                className="text-[9px] bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-2 rounded-xl font-black uppercase tracking-widest transition-all">
  ...
  const renderResumosTab = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[9px] text-slate-400 uppercase tracking-widest mt-1">Escolha um documento e o tipo de resumo desejado</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-4 space-y-3">
            <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest">1. Selecione o documento</h4>
            {knowledgeDocs.length === 0 ? (
              <p className="text-xs text-slate-400 italic">Nenhum documento na base legal. Adicione documentos na seção "Base Legal".</p>
            ) : (
                  ...
                  <button key={i} onClick={() => setSelectedDoc(doc)}
                    className={`w-full text-left p-3 rounded-xl transition-all border ${
                      selectedDoc?.id === doc.id
                        ...
                        : 'border-[#C9A84C]/30 hover:border-slate-600 bg-slate-900/30'
                    }`}>
                    <p className="text-xs font-bold text-white truncate">{doc.fileName || doc.title}</p>
                    <p className="text-[9px] text-slate-400 mt-0.5">{doc.content?.length || 0} caracteres indexados</p>
                  </button>
                ))}
              </div>
            )}
          ...
          {/* Seleção de tipo */}
          <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-4 space-y-3">
            <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest">2. Tipo de resumo</h4>
        ...
        <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-5 min-h-[350px] flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Resultado</h4>
            {summary && (
          ...
          {summaryLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center opacity-30">
              <div className="text-center space-y-2">
                <i className="fa-solid fa-file-lines text-4xl text-slate-300"></i>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Selecione um documento e gere o resumo</p>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="p-8 space-y-6 bg-[#0D1B3E] min-h-screen animate-in fade-in">
      ...
      <header>
        ...
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em]">MJ Consultoria // Capacitação Notarial IA</p>
      </header>
