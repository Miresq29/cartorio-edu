    ...
    return (
      <div className="space-y-6">
        ...
        <div className={`rounded-2xl p-8 text-center border ${quizResult.aprovado ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
          ...
          <p className="text-slate-400 text-xs mt-1">{corretas} de {selectedQuiz.questoes.length} corretas · Mínimo: 70%</p>
        </div>
              ...
              <div key={i} className={`p-5 rounded-2xl border space-y-3 ${acertou ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${acertou ? 'bg-emerald-500' : 'bg-red-500'}`}>
                    <i className={`fa-solid text-white text-xs ${acertou ? 'fa-check' : 'fa-xmark'}`}></i>
                  </div>
                  <p className="text-sm text-white font-bold">{i + 1}. {q.texto}</p>
                </div>
                ...
                {q.explicacao && (
                  <div className={`ml-9 p-3 rounded-xl text-xs leading-relaxed ${
                    acertou
                      ? 'bg-slate-800/50 text-slate-500 border border-[#C9A84C]/20'
                      : 'bg-amber-500/10 text-amber-300 border border-amber-500/20'
                  }`}>
                    <i className={`fa-solid fa-circle-info mr-2 ${acertou ? 'text-slate-400' : 'text-amber-400'}`}></i>
                    {q.explicacao}
                  </div>
                )}
              </div>
        ...
        <div className="flex gap-3">
          <button onClick={() => setMode('list')}
            className="flex-1 bg-slate-800 hover:bg-slate-700 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
      ...
      <div className="space-y-6 max-w-lg mx-auto">
        <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-8 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">{selectedQuiz.titulo}</p>
          </div>
          <div className="bg-[#0D1B3E] border border-red-500/20 rounded-xl p-5 space-y-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Nova tentativa disponível em</p>
        ...
        <div className="space-y-6 max-w-lg mx-auto">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Seu Nome *</label>
          ...
          <div className="flex gap-3">
            <button onClick={() => setMode('list')} className="px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 border border-[#C9A84C]/30 hover:border-slate-600 transition-all">
              Cancelar
            </button>
            <button onClick={() => {
      ...
      <div className="space-y-6 max-w-lg mx-auto">
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-400">{currentQ + 1} / {selectedQuiz.questoes.length}</span>
          </div>
        ...
        <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-6 space-y-4">
          <p className="text-white font-bold text-sm leading-relaxed">{currentQ + 1}. {questao.texto}</p>
          <div className="space-y-2">
            {questao.opcoes.map((opcao, i) => (
              <button key={i} onClick={() => handleAnswer(i)}
                className="w-full text-left p-4 rounded-xl bg-slate-900 border border-[#C9A84C]/30 hover:border-blue-500 hover:bg-blue-500/10 text-slate-200 text-sm transition-all">
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => setMode('list')} className="text-slate-400 hover:text-white transition-colors">
            <i className="fa-solid fa-arrow-left"></i>
          </button>
          <div>
            <h3 className="text-white font-black uppercase italic text-sm">Novo Questionário</h3>
          </div>
        </div>
          ...
          {(['ia', 'manual'] as const).map(m => (
            <button key={m} onClick={() => setCreateMode(m)}
              className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                createMode === m ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'
              }`}>
          ...
          <div className="space-y-5 max-w-lg">
            ...
            <div className="space-y-1">
              ...
              <select value={aiForm.treinamento} onChange={e => setAiForm(f => ({ ...f, treinamento: e.target.value }))}
                className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500">
                <option value="">Selecione o treinamento...</option>
                {checklists.map(c => <option key={c.id} value={c.title}>{c.title}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Quantidade de Questões</label>
        ...
        {createMode === 'manual' && (
          <div className="space-y-5">
            ...
            {questoes.map((q, qi) => (
              <div key={qi} className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-5 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Questão {qi + 1}</span>
                  {questoes.length > 1 && (
                    <button onClick={() => removeQuestao(qi)} className="text-slate-300 hover:text-red-400 transition-colors">
                    ...
                    <div key={oi} className="flex items-center gap-2">
                      <button onClick={() => updateQuestao(qi, 'correta', oi)}
                        className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${
                          q.correta === oi ? 'border-emerald-500 bg-emerald-500' : 'border-[#C9A84C]/20 hover:border-slate-500'
                        }`}>
                      ...
                      <input value={opcao} onChange={e => updateOpcao(qi, oi, e.target.value)}
                        ...
                        className="flex-1 bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-blue-500" />
                    </div>
                  ))}
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Explicação da resposta correta</label>
                  <textarea value={q.explicacao} onChange={e => updateQuestao(qi, 'explicacao', e.target.value)}
                    placeholder="Explique por que esta é a resposta correta..."
                    rows={2}
                    className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500 resize-none" />
                </div>
              </div>
            ))}
            ...
            <div className="flex gap-3">
              <button onClick={addQuestao}
                className="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 border border-[#C9A84C]/30 hover:border-slate-600 hover:text-white transition-all">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[9px] text-slate-400 uppercase tracking-widest mt-1">Criados pela IA ou manualmente</p>
        </div>
      ...
      {quizzes.length === 0 ? (
        <div className="bg-[#0D1B3E] border border-[#C9A84C]/30 rounded-2xl p-10 text-center">
          <i className="fa-solid fa-circle-question text-4xl text-slate-200 mb-3 block"></i>
          <p className="text-slate-300 text-xs font-bold uppercase tracking-widest">Nenhum questionário criado ainda</p>
          <p className="text-slate-200 text-xs mt-1">Clique em "Novo Questionário" e deixe a IA criar para você</p>
        </div>
      ) : (
        <div className="space-y-3">
          {quizzes.map(quiz => {
            ...
            return (
              <div key={quiz.id} className={`bg-[#0D1B3E] border rounded-2xl p-5 transition-all group ${bloqueado ? 'border-red-500/30' : 'border-[#C9A84C]/30 hover:border-[#C9A84C]/20'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-white font-black text-sm">{quiz.titulo}</span>
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <button type="button" title="Excluir questionário" onClick={() => deleteQuiz(quiz.id)} className="text-slate-300 hover:text-red-400 transition-colors p-2">
                      <i className="fa-solid fa-trash text-xs"></i>
                    </button>
                  </div>
                  <i className="fa-solid fa-chevron-right text-slate-200 text-xs ml-3 group-hover:opacity-0 transition-all"></i>
                </div>
              </div>
            );
          })}
