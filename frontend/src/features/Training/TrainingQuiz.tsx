...
const TrainingQuiz: React.FC<Props> = ({ checklists }) => {
  ...
  const generateQuizWithAI = async () => {
    ...
    const prompt = `VocÃª Ã© um especialista em treinamento notarial da MJ Consultoria.
Com base no seguinte treinamento, crie ${aiForm.quantidade} questÃµes de mÃºltipla escolha para avaliar o conhecimento dos colaboradores.

${context}

RESPONDA APENAS COM UM JSON VÃLIDO, sem texto antes ou depois, sem markdown, sem \`\`\`:
{
  "titulo": "AvaliaÃ§Ã£o de [nome do treinamento]",
  "questoes": [
    {
      "texto": "Texto da pergunta",
      "opcoes": ["OpÃ§Ã£o A", "OpÃ§Ã£o B", "OpÃ§Ã£o C", "OpÃ§Ã£o D"],
      "correta": 0,
      "explicacao": "ExplicaÃ§Ã£o detalhada do porquÃª esta Ã© a resposta correta e por que as outras estÃ£o erradas"
    }
  ]
}

Regras:
- "correta" Ã© o Ã­ndice (0 a 3) da opÃ§Ã£o correta
- As perguntas devem ser prÃ¡ticas e relevantes para o dia a dia do cartÃ³rio
- A explicaÃ§Ã£o deve ser clara e didÃ¡tica, com no mÃ­nimo 2 linhas
- Retorne exatamente ${aiForm.quantidade} questÃµes`;
    ...
    try {
      ...
      if (!jsonMatch) throw new Error('Resposta invÃ¡lida da IA');
      ...
      setGenerateError('Erro ao gerar questionÃ¡rio. Tente novamente.');
      console.error(e);
    } finally {
  ...
  const startQuiz = (quiz: Quiz) => {
    ...
    // Verifica bloqueio de 3 dias para o usuÃ¡rio logado
    if (nomeAtual) {
  ...
  const deleteQuiz = async (id: string) => {
    if (!window.confirm('Excluir este questionÃ¡rio?')) return;
    await deleteDoc(doc(db, 'treinamentosQuizzes', id));
  };

  // ---- TELA DE RESULTADO COM EXPLICAÃ‡Ã•ES ----
    ...
    return (
      <div className="space-y-6">
        ...
        <div className={`rounded-2xl p-8 text-center border ${quizResult.aprovado ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
          ...
          <p className="text-slate-500 text-sm mt-1">{quizResult.colaborador} â€” {quizResult.treinamento}</p>
          <p className="text-slate-500 text-xs mt-1">{corretas} de {selectedQuiz.questoes.length} corretas Â· MÃ­nimo: 70%</p>
        </div>
          ...
          {selectedQuiz.questoes.map((q, i) => {
            ...
            return (
              <div key={i} className={`p-5 rounded-2xl border space-y-3 ${acertou ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                ...
                {/* ExplicaÃ§Ã£o sempre visÃ­vel, destacada quando errou */}
                {q.explicacao && (
    ...
    return (
      <div className="space-y-6 max-w-lg mx-auto">
        <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-8 text-center space-y-4">
          ...
          <div>
            <h3 className="text-red-400 font-black uppercase italic text-lg">AvaliaÃ§Ã£o Bloqueada</h3>
            <p className="text-sm text-slate-500 mt-2">
              VocÃª nÃ£o atingiu a nota mÃ­nima de <span className="font-black text-[#0A1628]">70%</span> na tentativa anterior.
            </p>
          ...
          <div className="bg-[#0D1B3E] border border-red-500/20 rounded-xl p-5 space-y-1">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Nova tentativa disponÃ­vel em</p>
            <p className="text-4xl font-black text-red-400">{formatCountdown(bloqueadoAte)}</p>
            <p className="text-[9px] text-slate-600">
              Liberado em {bloqueadoAte.toLocaleDateString('pt-BR')} Ã s {bloqueadoAte.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
            </p>
          </div>
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-left">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Use o tempo de espera para revisar o conteÃºdo do treinamento <span className="font-bold text-[#0A1628]">{selectedQuiz.treinamento}</span> na base legal e nos checklists.
            </p>
          </div>
          <button type="button" onClick={() => { setMode('list'); setBloqueadoAte(null); }}
            ...
            <i className="fa-solid fa-arrow-left mr-2"></i>Voltar para AvaliaÃ§Ãµes
          </button>
        </div>
      </div>
    );
        ...
        <div className="space-y-6 max-w-lg mx-auto">
          <div>
            ...
            <p className="text-[9px] text-slate-500 uppercase tracking-widest mt-1">
              {selectedQuiz.treinamento} Â· {selectedQuiz.questoes.length} questÃµes Â· AprovaÃ§Ã£o: 70%
            </p>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => {
              ...
              <i className="fa-solid fa-play mr-2"></i>Iniciar AvaliaÃ§Ã£o
            </button>
          </div>
        </div>
      );
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[#0A1628] font-black uppercase italic text-sm">Novo QuestionÃ¡rio</h3>
          </div>
        </div>
          ...
          <div className="space-y-5 max-w-lg">
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4">
              <p className="text-xs text-blue-300">
                ...
                A IA criarÃ¡ questÃµes baseadas no conteÃºdo do treinamento selecionado, incluindo explicaÃ§Ãµes para cada resposta.
              </p>
            </div>
            <div className="space-y-1">
              ...
              <select value={aiForm.treinamento} onChange={e => setAiForm(f => ({ ...f, treinamento: e.target.value }))}
                className="w-full bg-slate-900 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-[#0A1628] outline-none focus:border-blue-500">
                <option value="">Selecione o treinamento...</option>
                {checklists.map(c => <option key={c.id} value={c.title}>{c.title}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-500">Quantidade de QuestÃµes</label>
              <select value={aiForm.quantidade} onChange={e => setAiForm(f => ({ ...f, quantidade: e.target.value }))}
                className="w-full bg-slate-900 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-[#0A1628] outline-none focus:border-blue-500">
                {['3', '5', '7', '10'].map(n => <option key={n} value={n}>{n} questÃµes</option>)}
              </select>
            </div>
            ...
            <button onClick={generateQuizWithAI} disabled={isGenerating || !aiForm.treinamento}
              ...
              {isGenerating
                ...
                : <><i className="fa-solid fa-wand-magic-sparkles"></i>Gerar QuestionÃ¡rio</>
              }
            </button>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input value={form.titulo} onChange={e => setForm(f => ({ ...f, titulo: e.target.value }))}
                  placeholder="Ex: AvaliaÃ§Ã£o NR-35"
                  className="w-full bg-slate-900 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-[#0A1628] outline-none focus:border-blue-500" />
              </div>
            </div>
            ...
            {questoes.map((q, qi) => (
              <div key={qi} className="bg-[#0D1B3E] border border-slate-200 rounded-2xl p-5 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">QuestÃ£o {qi + 1}</span>
                  {questoes.length > 1 && (
                  ...
                  {q.opcoes.map((opcao, oi) => (
                    <div key={oi} className="flex items-center gap-2">
                      ...
                      <input value={opcao} onChange={e => updateOpcao(qi, oi, e.target.value)}
                        placeholder={`OpÃ§Ã£o ${String.fromCharCode(65 + oi)}`}
                        className="flex-1 bg-slate-900 border border-slate-200 rounded-xl px-3 py-2 text-sm text-[#0A1628] outline-none focus:border-blue-500" />
                    </div>
                  ))}
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-500">ExplicaÃ§Ã£o da resposta correta</label>
                  <textarea value={q.explicacao} onChange={e => updateQuestao(qi, 'explicacao', e.target.value)}
                    placeholder="Explique por que esta Ã© a resposta correta..."
                    rows={2}
                    className="w-full bg-slate-900 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-[#0A1628] outline-none focus:border-blue-500 resize-none" />
                </div>
              </div>
            ))}
            ...
            <div className="flex gap-3">
              <button onClick={addQuestao}
                ...
                <i className="fa-solid fa-plus mr-2"></i>QuestÃ£o
              </button>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-[#0A1628] font-black uppercase italic text-sm">
            QuestionÃ¡rios de <span className="text-blue-500">AvaliaÃ§Ã£o</span>
          </h3>
        ...
        <button onClick={() => setMode('create')}
          ...
          <i className="fa-solid fa-plus"></i>Novo QuestionÃ¡rio
        </button>
      </div>
      ...
      {quizzes.length === 0 ? (
        <div className="bg-[#0D1B3E] border border-slate-200 rounded-2xl p-10 text-center">
          ...
          <p className="text-slate-600 text-xs font-bold uppercase tracking-widest">Nenhum questionÃ¡rio criado ainda</p>
          <p className="text-slate-700 text-xs mt-1">Clique em "Novo QuestionÃ¡rio" e deixe a IA criar para vocÃª</p>
        </div>
      ) : (
        <div className="space-y-3">
          {quizzes.map(quiz => {
              ...
              <div key={quiz.id} className={`bg-[#0D1B3E] border rounded-2xl p-5 transition-all group ${bloqueado ? 'border-red-500/30' : 'border-slate-200 hover:border-slate-200'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded-md">
                        {quiz.questoes.length} questÃµes
                      </span>
                      {quiz.geradoPorIA && (
                      ...
                      {bloqueado && (
                        <span className="text-[9px] font-black uppercase tracking-widest text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded-md">
                          <i className="fa-solid fa-lock mr-1"></i>Bloqueado Â· {formatCountdown(bloqueado)}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-4 mt-1">
                      ...
                      <span className="text-xs text-slate-600">
                        {qtdResultados} realizaÃ§Ãµes Â· {aprovados} aprovaÃ§Ãµes
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <button type="button" title="Excluir questionÃ¡rio" onClick={() => deleteQuiz(quiz.id)} className="text-slate-600 hover:text-red-400 transition-colors p-2">
                      <i className="fa-solid fa-trash text-xs"></i>
                    </button>
                  </div>
