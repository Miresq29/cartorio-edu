...
const Tip: React.FC<any> = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-[#E8D5A3] rounded-xl p-3 shadow-lg text-xs">
      {label && <p className="font-black text-[#1A2744] mb-1">{label}</p>}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm font-black text-[#0A1628]">Progresso Geral do Programa</span>
          <span className="text-sm font-black text-[#1A3A6B]">{globalPct}%</span>
        </div>
        <div className="w-full bg-[#F5EDD8] rounded-full h-2.5">
          <div className="h-2.5 rounded-full bg-[#1A3A6B] transition-all duration-700" style={{ width: `${globalPct}%` }}></div>
        </div>
        <p className="text-[11px] text-[#8A9BB0] mt-2">Aprovação mínima: <strong className="text-[#2C3E5A]">75%</strong> · Taxonomia de Bloom Médio</p>
      </div>

      {/* Linha 1: Evolução + Donut */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Evolução das Notas</p>
          {evolucao.length === 0 ? (
            <div className="h-48 flex items-center justify-center text-[#8A9BB0] text-sm">Nenhum teste realizado ainda</div>
          ) : (
        ...
        <div className="bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Status das Trilhas</p>
            ...
            {statusTrilhas.map((s, i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-[#5A6E8A]">{s.name}</span>
                </div>
                <span className="text-[11px] font-black text-[#1A2744]">{s.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Conclusão e Notas por Trilha</p>
          {porTrilha.length === 0 ? (
            <div className="h-48 flex items-center justify-center text-[#8A9BB0] text-sm">Sem dados ainda</div>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-2">Perfil de Habilidades</p>
      ...
      {/* Trilhas */}
      <div>
        <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-3">Trilhas de Aprendizado</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {trilhas.map((t, idx) => {
            ...
            return (
              <div key={t.id} className="bg-white border border-[#E8D5A3] hover:border-[#D4B86A] rounded-[14px] p-5 shadow-sm hover:shadow-md transition-all cursor-pointer">
                <div className="flex items-center justify-between mb-3">
                ...
                <h3 className="text-sm font-black text-[#0A1628] mb-1 leading-snug">{t.titulo}</h3>
                <p className="text-[11px] text-[#8A9BB0] mb-3 line-clamp-2">{t.descricao}</p>
                <p className="text-[10px] text-[#8A9BB0] mb-2">{done}/{mods} módulos</p>
                <div className="w-full bg-[#F5EDD8] rounded-full h-1.5">
      ...
      {myRes.length > 0 && (
        <div>
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-3">Atividade Recente</p>
          <div className="space-y-2">
            {myRes.slice(0, 5).map((r, i) => (
              <div key={i} className="bg-white border border-[#E8D5A3] rounded-[12px] p-4 flex items-center gap-3 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <p className="text-[10px] text-[#8A9BB0]">{formatDate(r.createdAt)}</p>
                    {r.ia && <span className="text-[9px] font-black bg-[#FBF5E0] text-[#1A3A6B] px-1.5 py-0.5 rounded-lg">✨ IA</span>}
                  </div>
                </div>
...
const AdminDashboard: React.FC<{
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Atividade Mensal da Equipe</p>
        ...
        <div className="bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Distribuição de Notas</p>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
          ...
          <div className="grid grid-cols-2 gap-1 mt-2">
            {distNotas.map((d, i) => (
              <div key={i} className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-[#5A6E8A]">{d.faixa}%: <strong className="text-[#1A2744]">{d.value}</strong></span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Taxa de Aprovação por Trilha</p>
        ...
        <div className="bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Perfis de Acesso</p>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
            ...
            {perfisData.map((p, i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-[#5A6E8A]">{p.name}</span>
                </div>
                <span className="text-[11px] font-black text-[#1A2744]">{p.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {topColab.length > 0 && (
        <div className="bg-white border border-[#E8D5A3] rounded-[14px] p-5 shadow-sm">
          <p className="text-xs font-black text-[#5A6E8A] uppercase tracking-widest mb-4">Ranking de Desempenho — Top Colaboradores</p>
          <ResponsiveContainer width="100%" height={220}>
...
const DashboardView: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#FBF7EE]">
        <div className="text-center space-y-3">
          <div className="w-10 h-10 border-2 border-[#C9A84C]/30 border-t-indigo-500 rounded-full animate-spin mx-auto"></div>
          <p className="text-[#8A9BB0] text-sm">Carregando dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FBF7EE]">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2 bg-[#FBF5E0] border border-[#E8D5A3] px-3 py-2 rounded-xl">
            <i className="fa-solid fa-brain text-[#C9A84C] text-xs"></i>
            <span className="text-[10px] font-black text-[#1A3A6B] uppercase tracking-widest">Bloom Médio · 75% aprovação</span>
          </div>
        </div>
