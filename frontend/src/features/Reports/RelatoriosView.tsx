    ...
    <div className="min-h-screen bg-[#F5F4EF]">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black text-slate-800">Relatórios de Treinamento</h2>
            <p className="text-sm text-slate-500 mt-0.5">Desempenho · Evidências · Exportação</p>
          </div>
          <div className="flex items-center gap-3">
            <select value={periodo} onChange={e => setPeriodo(e.target.value)}
              className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 outline-none focus:border-indigo-500 shadow-sm">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 space-y-3">
                    ...
                    {porMes.length === 0 ? (
                      <div className="h-48 flex items-center justify-center text-slate-400 text-sm">Sem dados no período</div>
                    ) : (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={i} className="bg-[#F5F4EF] border border-slate-200 rounded-xl p-4 text-center">
            ...
            {tab === 'colaboradores' && (
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-400 font-bold">{porColab.length} colaboradores</span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {porColab.length === 0 && (
                        <tr><td colSpan={9} className="text-center p-8 text-slate-400">Nenhum dado encontrado.</td></tr>
                      )}
                      {porColab.map(c => (
                        <tr key={c.id} className="border-b border-slate-100 hover:bg-[#F5F4EF] transition-all">
              ...
              <div className="space-y-4">
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {porTrilha.length === 0 && (
                        <tr><td colSpan={6} className="text-center p-8 text-slate-400">Sem dados no período.</td></tr>
                      )}
                      {porTrilha.map((t, i) => {
                        ...
                        return (
                          <tr key={i} className="border-b border-slate-100 hover:bg-[#F5F4EF] transition-all">
                            ...
                            <td className="p-3">
                              {ia > 0
                                ? <span className="bg-indigo-50 text-indigo-600 text-[10px] font-black px-2 py-0.5 rounded-lg border border-indigo-100">✨ {ia}</span>
                                : <span className="text-slate-400">–</span>}
                            </td>
                          </tr>
                        );
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {filteredResults.length === 0 && (
                        <tr><td colSpan={7} className="text-center p-8 text-slate-400">Nenhum registro no período.</td></tr>
                      )}
                      {filteredResults.map(r => (
                        <tr key={r.id} className="border-b border-slate-100 hover:bg-[#F5F4EF] transition-all">
                          <td className="p-3 font-bold text-slate-800">{r.colaborador}</td>
                          ...
                          <td className="p-3 text-slate-400 whitespace-nowrap">{formatDate(r.createdAt)}</td>
