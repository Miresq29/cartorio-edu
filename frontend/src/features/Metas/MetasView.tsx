    ...
    <div className="bg-white border border-blue-500/30 rounded-2xl p-6 space-y-5">
      ...
      <div className="space-y-1">
        ...
        <textarea value={form.descricao} onChange={e => set('descricao', e.target.value)}
          ...
          rows={2}
          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-navy outline-none focus:border-blue-500 resize-none" />
      </div>
          ...
          {(Object.entries(METRICA_CONFIG) as [Metrica, typeof METRICA_CONFIG[Metrica]][]).map(([key, cfg]) => (
            <button type="button" key={key} onClick={() => set('metrica', key)}
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className={`text-xs font-black ${form.metrica === key ? `text-${cfg.color}-300` : 'text-navy'}`}>{cfg.label}</p>
              </div>
      ...
      <div className="p-5 cursor-pointer" onClick={() => setExpanded(e => !e)}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-navy font-black text-sm">{meta.titulo}</h3>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            {meuRanking >= 0 && (
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                meuRanking < meta.topN ? 'bg-amber-500/20 border border-amber-500/30' : 'bg-slate-200'
              }`}>
        ...
        <div className="border-t border-slate-200/50 p-5 space-y-5">
              ...
              <div className="flex-1">
                ...
                {!sessaoAberta && (
                  <button
                    ...
                    className="mt-2 bg-amber-600 hover:bg-amber-500 text-navy px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all">
                ...
                {marked.slice(0, 10).map((item, i) => {
                  ...
                  return (
                    <div key={item.nome} className={`flex items-center gap-3 p-2.5 rounded-xl transition-all ${
                      ...
                      <span className={`flex-1 text-sm font-bold truncate ${isMe ? 'text-blue-400' : 'text-navy'}`}>
                        {item.nome}
          ...
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-black text-navy">{meta.premio}</p>
              {meta.dataFim && (
            ...
            <div className="flex gap-2 flex-wrap border-t border-slate-200/50 pt-4">
              {meta.status === 'ativa' && (
                <button onClick={onEncerrar}
                  className="px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest bg-slate-200 hover:bg-slate-700 text-slate-700 transition-all">
  ...
  if (fase === 'resultado' && resultado) {
    return (
      <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-6">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center max-w-md space-y-4">
          ...
          <p className="text-navy font-black text-lg uppercase">Desempate concluído!</p>
      ...
      <div className="bg-white border border-amber-500/30 rounded-2xl w-full max-w-2xl space-y-5 p-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-2xl font-black text-navy">{Object.keys(respostas).length}<span className="text-slate-600">/{total}</span></p>
        ...
        <div className="space-y-4">
          {sessao.questoes.map((q, qi) => {
            ...
            return (
              <div key={q.id} className={`bg-white border rounded-2xl p-5 space-y-3 ${resp ? 'border-amber-500/30' : 'border-slate-200'}`}>
                  ...
                  {q.alternativas.map(alt => (
                    <button key={alt.letra} type="button"
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                        resp === alt.letra
                          ? 'border-amber-500 bg-amber-500/20 text-navy'
                          : 'border-slate-200 hover:border-slate-600 text-slate-700'
                      }`}>
                      <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black flex-shrink-0 ${
                        resp === alt.letra ? 'bg-amber-500 text-navy' : 'bg-slate-200 text-slate-500'
                      }`}>{alt.letra}</span>
        ...
        <button
          ...
          className="w-full py-4 bg-amber-600 hover:bg-amber-500 disabled:bg-slate-700 disabled:text-slate-500 text-navy font-black uppercase tracking-widest rounded-2xl transition-all text-sm">
...
[{
  ...
  "justificativa": "Explicação breve da resposta correta"
      ...
      const raw = await GeminiService.generateJSON(prompt, 3000);
      ...
      <div className="bg-purple-500/5 border border-purple-500/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
            ...
            <strong className="text-navy"> Nível 5 (Avaliar)</strong> e <strong className="text-navy">Nível 6 (Criar)</strong> da Taxonomia de Bloom — 
      ...
      {gerandoDesempate && (
        <div className="fixed inset-0 bg-black/70 z-40 flex items-center justify-center">
          <div className="bg-white border border-amber-500/30 rounded-2xl p-8 text-center space-y-4 max-w-sm">
            ...
            <p className="text-navy font-black text-sm uppercase tracking-widest">Gerando Quiz de Desempate</p>
