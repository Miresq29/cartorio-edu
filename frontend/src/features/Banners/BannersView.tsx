    ...
    <div className="p-8 space-y-6 bg-[#0D1B3E] min-h-screen animate-in fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#1A2A52] border border-[#C9A84C]/30 rounded-[24px] p-6 space-y-4">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-blue-500 transition-colors"
            />
          ...
          <div className="bg-white border border-pink-500/30 rounded-[24px] p-6 space-y-4">
            ...
            <textarea
              ...
              className="w-full bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-pink-500 resize-none"
            />
        ...
        return (
          <div className="bg-white border border-pink-500/30 rounded-[24px] p-6 space-y-4">
            ...
            <div className="bg-gradient-to-br from-blue-900 to-slate-900 rounded-2xl p-10 text-center space-y-4 border border-blue-500/20">
              ...
              <p className="text-slate-200 text-sm md:text-base leading-relaxed max-w-lg mx-auto">{subtitulo}</p>
              {cta && (
            ...
            <div className="flex gap-3 flex-wrap">
              <input
                ...
                className="flex-1 min-w-0 bg-slate-900 border border-[#C9A84C]/30 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-pink-500"
              />
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <button type="button" onClick={() => setFiltroTipo('')}
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
            !filtroTipo ? 'bg-pink-600 text-white' : 'bg-slate-900 text-slate-500 hover:bg-slate-800'
          }`}>
        ...
        {Object.entries(TIPO_CONFIG).map(([id, cfg]) => (
          <button type="button" key={id} onClick={() => setFiltroTipo(filtroTipo === id ? '' : id)}
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
              filtroTipo === id ? 'bg-pink-600 text-white' : 'bg-slate-900 text-slate-500 hover:bg-slate-800'
            }`}>
      ...
      {filtrados.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 opacity-30">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Nenhum material encontrado</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtrados.map(m => {
            ...
            return (
              <div key={m.id}
                className="bg-[#1A2A52] border border-[#C9A84C]/30 rounded-[20px] p-5 space-y-3 hover:border-[#C9A84C]/20 transition-all">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-slate-400 uppercase tracking-widest">{cfg.label}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {m.linkUrl && (
                    <a href={m.linkUrl} target="_blank" rel="noreferrer"
                      className="flex-1 text-center text-[9px] bg-slate-800 hover:bg-slate-700 text-slate-200 px-3 py-2 rounded-xl font-black uppercase tracking-widest transition-all">
                  ...
                  {m.textoBanner && (
                    <button type="button" onClick={() => imprimirBanner(m.textoBanner)}
                      className="flex-1 text-[9px] bg-slate-800 hover:bg-slate-700 text-slate-200 px-3 py-2 rounded-xl font-black uppercase tracking-widest transition-all">
                      <i className="fa-solid fa-print mr-1"></i>Imprimir
                    </button>
                  )}
                  {isGestor && (
                    <button type="button" title="Excluir material" onClick={() => excluir(m.id)}
                      className="w-8 h-8 bg-slate-900 hover:bg-red-500/20 text-slate-400 hover:text-red-400 rounded-xl flex items-center justify-center transition-all flex-shrink-0">
