    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 pb-32 animate-in fade-in duration-700">
      ...
      <div className="bg-blue-600/5 border border-blue-500/10 rounded-2xl p-6 flex items-start gap-5 no-print">
        ...
        <div>
          ...
          <p className="text-sm text-[#8A9BB0] mt-2 leading-relaxed font-medium italic">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 no-print">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#5A6E8A] font-bold uppercase tracking-widest mt-1">Tenant: {state.user?.tenantId}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 no-print">
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-[#FBF7EE] border border-[#E8D5A3] rounded-[32px] p-8 space-y-7 shadow-2xl">

            <div className="space-y-3">
              <label className="text-[10px] font-black text-[#5A6E8A] uppercase tracking-widest ml-1">Documento a Auditar</label>
              ...
              <button
                ...
                className={`w-full py-10 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center gap-3 transition-all ${docData ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-[#E8D5A3] hover:border-blue-500/50'}`}
              >
                <i className={`fa-solid ${docData ? 'fa-file-circle-check text-emerald-500' : 'fa-cloud-arrow-up text-[#2C3E5A]'} text-4xl`}></i>
                <p className="text-xs font-bold text-[#1A2744] truncate max-w-xs px-4">{docData ? docData.fileName : 'Subir Minuta ou Documento'}</p>
                {docData && (
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-[#5A6E8A] uppercase tracking-widest ml-1">Analisar com base em</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => setAnalysisMode('checklist')}
                  className={`py-3 rounded-xl text-[10px] font-black uppercase transition-all ${analysisMode === 'checklist' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-[#8A9BB0] hover:bg-slate-700'}`}
                >
                  <i className="fa-solid fa-clipboard-list mr-2"></i>Protocolo
                </button>
                <button
                  onClick={() => setAnalysisMode('baselegal')}
                  className={`py-3 rounded-xl text-[10px] font-black uppercase transition-all ${analysisMode === 'baselegal' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-[#8A9BB0] hover:bg-slate-700'}`}
                >
            ...
            {analysisMode === 'checklist' && (
              <div className="space-y-3">
                <label className="text-[10px] font-black text-[#5A6E8A] uppercase tracking-widest ml-1">Protocolo de Referência</label>
                {checklists.length === 0 ? (
                  <p className="text-xs text-[#2C3E5A] italic p-4 border border-[#E8D5A3] rounded-xl">Nenhum protocolo cadastrado. Crie um no módulo Checklists.</p>
                ) : (
                  <select
                    value={selectedChecklistId}
                    onChange={e => setSelectedChecklistId(e.target.value)}
                    className="w-full bg-[#FBF7EE] border border-[#E8D5A3] rounded-xl px-4 py-4 text-xs font-bold text-[#0A1628] focus:border-blue-600 outline-none transition-all"
                  >
            ...
            {analysisMode === 'baselegal' && (
              <div className="space-y-3">
                <label className="text-[10px] font-black text-[#5A6E8A] uppercase tracking-widest ml-1">
                  Documentos da Base Legal ({selectedKbIds.length} selecionado{selectedKbIds.length !== 1 ? 's' : ''})
                </label>
                {kbDocs.length === 0 ? (
                  <p className="text-xs text-[#2C3E5A] italic p-4 border border-[#E8D5A3] rounded-xl">Nenhum documento indexado. Adicione documentos na Base Legal.</p>
                ) : (
                  <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                    {kbDocs.map(d => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all border ${selectedKbIds.includes(d.id) ? 'border-blue-500/40 bg-blue-500/5' : 'border-[#E8D5A3] hover:border-[#D4C5A0]'}`}
                      >
                        ...
                        <span className="text-[10px] text-[#1A2744] font-bold truncate">{d.fileName}</span>
                      </button>
                    ))}
                  </div>
                )}
        ...
        <div className="lg:col-span-7 flex flex-col justify-center items-center text-center p-10 bg-blue-600/5 border border-blue-500/10 rounded-[32px] shadow-inner">
          ...
          <p className="text-[#8A9BB0] text-sm mt-4 max-w-md leading-relaxed">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 w-full max-w-sm">
            <div className="bg-slate-900 rounded-2xl p-4 text-center">
              ...
              <p className="text-[9px] text-[#5A6E8A] uppercase font-bold mt-1">Protocolos</p>
            </div>
            <div className="bg-slate-900 rounded-2xl p-4 text-center">
              <p className="text-2xl font-black text-blue-500">{kbDocs.length}</p>
              <p className="text-[9px] text-[#5A6E8A] uppercase font-bold mt-1">Docs Indexados</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {results.length > 0 && (
        <div className="results-report mt-8 space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white border border-[#E8D5A3] rounded-2xl p-6 text-center">
              <p className="text-3xl font-black text-white">{results.length}</p>
              <p className="text-[9px] text-[#5A6E8A] uppercase font-bold mt-1">Total de Requisitos</p>
            </div>
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-6 text-center">
              ...
              <p className="text-[9px] text-[#5A6E8A] uppercase font-bold mt-1">Conformes</p>
            </div>
            <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-6 text-center">
              ...
              <p className="text-[9px] text-[#5A6E8A] uppercase font-bold mt-1">Pendentes</p>
            </div>
          </div>

          <div className="bg-white border border-[#E8D5A3] rounded-2xl p-6">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[10px] font-black text-[#5A6E8A] uppercase tracking-widest">Índice de Conformidade</span>
            ...
            <div className="flex justify-between mt-2">
              <span className="text-[9px] text-[#2C3E5A]">Auditado em: {auditDate}</span>
              <span className="text-[9px] text-[#2C3E5A]">{docData?.fileName}</span>
            </div>
          </div>

          <div className="p-8 bg-slate-900/40 border border-[#E8D5A3] rounded-3xl backdrop-blur-md">
            ...
            <div className="space-y-4">
              {results.map((res, i) => (
                <div key={i} className={`p-6 rounded-2xl border ${res.compliant ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'} transition-all`}>
                  ...
                  <p className="text-xs text-[#8A9BB0] leading-relaxed mb-3">{res.comment}</p>
                  {!res.compliant && res.suggestion && (
                    <div className="bg-slate-950 p-4 rounded-xl border border-red-500/10">
                      ...
                      <p className="text-xs text-[#1A2744] leading-relaxed">{res.suggestion}</p>
                    </div>
                  )}
                </div>
              ))}
          ...
          {rawParecer && (
            <div className="p-8 bg-white border border-[#D4C5A0] rounded-3xl">
              ...
              <pre className="text-xs text-[#1A2744] leading-relaxed whitespace-pre-wrap font-sans">{rawParecer}</pre>
            </div>
          )}
        </div>
      )}
      ...
      {!results.length && rawParecer && (
        <div className="mt-8 p-8 bg-slate-900/40 border border-[#E8D5A3] rounded-3xl">
          ...
          <pre className="text-sm text-[#1A2744] leading-relaxed whitespace-pre-wrap font-sans">{rawParecer}</pre>
        </div>
      )}
