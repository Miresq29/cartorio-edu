          ...
          {checklists.map(t => (
            <div key={t.id} className="group flex items-center gap-2">
              <button
                ...
                className={`flex-1 p-4 rounded-xl flex items-center gap-3 transition-all text-left ${activeChecklistId === t.id ? 'bg-blue-600 text-white' : 'hover:bg-slate-100 text-slate-500'}`}
              >
        ...
        {!activeChecklist ? (
          ...
          <div className="p-10 space-y-6 max-w-5xl mx-auto">
            ...
            <div className="flex items-start justify-between flex-wrap gap-4">
              <div>
                <h3 className="text-3xl font-black text-[#0A1628] italic uppercase">{activeChecklist.title}</h3>
              ...
              <div className="flex gap-2">
                <button onClick={() => setMode('roteiro')}
                  className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${mode === 'roteiro' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 border border-slate-200'}`}>
                  <i className="fa-solid fa-list mr-2"></i>Roteiro
                </button>
                <button onClick={() => setMode('executar')}
                  className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${mode === 'executar' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 border border-slate-200'}`}>
            ...
            {mode === 'roteiro' && (
              <div className="bg-white border border-slate-200 rounded-[32px] p-8 space-y-3 shadow-2xl">
                ...
                {activeChecklist.items?.map((item, idx) => (
                  <div key={item.id} className="flex items-start gap-4 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                    <div className="w-7 h-7 rounded-lg bg-slate-200 flex items-center justify-center text-blue-500 text-xs font-bold flex-shrink-0 mt-0.5">{idx + 1}</div>
                    ...
                    <div key={item.id} className={`border rounded-2xl p-5 transition-all ${checkedItems[item.id] ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-white border-slate-200'}`}>
                      <div className="flex items-start gap-4">
                        <button
                          ...
                          className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 transition-all border-2 ${checkedItems[item.id] ? 'bg-emerald-500 border-emerald-500 text-[#0A1628]' : 'border-slate-600 hover:border-blue-500'}`}
                        >
                          {checkedItems[item.id] && <i className="fa-solid fa-check text-xs"></i>}
                        </button>
                        <div className="flex-1">
                          <span className={`text-sm font-medium leading-relaxed ${checkedItems[item.id] ? 'text-slate-500 line-through' : 'text-slate-700'}`}>
                          ...
                          <textarea
                            ...
                            rows={1}
                            className="mt-2 w-full bg-white border border-slate-200 rounded-lg p-2 text-xs text-slate-500 outline-none focus:border-blue-500 resize-none"
                          />
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-white border border-blue-500/20 rounded-[32px] p-8 space-y-5">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-5 py-3 rounded-xl text-[10px] font-black uppercase transition-all border ${uploadedDocText ? 'border-emerald-500/40 text-emerald-400 bg-emerald-500/5' : 'border-slate-200 text-slate-500 hover:border-blue-500 hover:text-[#0A1628]'}`}
                    >
                  ...
                  {analysisResult && (
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                      ...
                      <pre className="text-xs text-slate-700 leading-relaxed whitespace-pre-wrap font-sans">{analysisResult}</pre>
                    </div>
                  )}
                </div>
              </div>
