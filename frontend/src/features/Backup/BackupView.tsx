  ...
  return (
    <div className="min-h-screen bg-[#0D1B3E]">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5">
              Exportação segura dos dados do cartório <strong className="text-slate-200">{tenantId}</strong>
            </p>
          </div>
          {ultimoBackup && (
            <div className="text-right">
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black">Último backup</p>
              <p className="text-xs text-slate-300 font-bold">{ultimoBackup}</p>
            </div>
          )}
        </div>
        ...
        {/* Botão principal */}
        {isGestor && (
          <div className="bg-[#1A2A52] border border-[#C9A84C]/30 rounded-[16px] p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-black text-white">Backup Completo</h3>
              ...
              <button
                ...
                disabled={rodando}
                className="flex items-center gap-2 bg-[#C9A84C] hover:bg-[#A8863C] disabled:bg-slate-300 text-white px-6 py-3 rounded-xl text-sm font-bold transition-all shadow-sm">
                {rodando
        ...
        <div className="bg-[#1A2A52] border border-[#C9A84C]/30 rounded-[16px] shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h3 className="text-sm font-black text-white">Backup por Coleção</h3>
            <p className="text-xs text-slate-400 mt-0.5">Exporte uma coleção específica de forma individual</p>
          </div>
              ...
              return (
                <div key={col.id} className="flex items-center justify-between p-4 hover:bg-[#0D1B3E] transition-all">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-[#132044] rounded-xl flex items-center justify-center flex-shrink-0">
                      <i className={`fa-solid ${col.icon} text-slate-400 text-sm`}></i>
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-200">{col.label}</p>
                  ...
                  <div className="flex items-center gap-3">
                    {status && (
                      <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${
                        ...
                        status.status === 'vazio' ? 'bg-[#132044] text-slate-500 border border-[#C9A84C]/30' :
                    ...
                    {isGestor && (
                      <button
                        ...
                        disabled={rodando}
                        className="flex items-center gap-1.5 bg-[#1A2A52] border border-[#C9A84C]/30 hover:border-indigo-400 hover:text-[#C9A84C] text-slate-400 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-40">
