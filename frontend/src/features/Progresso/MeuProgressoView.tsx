...
const BadgeCard: React.FC<{ badge: BadgeDef; earned: boolean }> = ({ badge, earned }) => {
  ...
  return (
    <div className={'rounded-[14px] border p-4 flex flex-col items-center gap-2 text-center transition-all ' +
      (earned ? 'bg-white border-slate-200 shadow-sm hover:shadow-md' : 'bg-slate-100/50 border-slate-200 opacity-40 grayscale')}>
      ...
      <div>
        <p className="text-xs font-black text-slate-800 leading-tight">{badge.nome}</p>
      ...
      {!earned && <span className="text-[9px] text-slate-400 font-bold">Bloqueado</span>}
    </div>
  );
};
...
const MeuProgressoView: React.FC = () => {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-screen bg-[#F5F4EF]">
    ...
    <div className="min-h-screen bg-[#F5F4EF]">
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="ml-auto hidden md:flex items-center gap-2 bg-indigo-50 border border-indigo-200 px-3 py-2 rounded-xl">
            <i className="fa-solid fa-trophy text-indigo-500 text-xs"></i>
            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">{earnedBadges.length} conquistas</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[14px] p-5 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-black text-slate-800">Progresso no Programa</span>
          ...
          <div className="flex items-center gap-4 mt-3">
            <span className="text-[11px] text-slate-400">Aprovação mínima: <strong className="text-slate-600">75%</strong></span>
            <span className="text-[11px] text-slate-400">Badges: <strong className="text-slate-600">{earnedBadges.length}/{totalBadges}</strong></span>
            <span className="text-[11px] text-slate-400">Certificados: <strong className="text-slate-600">{myCerts.length}</strong></span>
          </div>
        </div>
            ...
            {[
              ...
              <button key={t.id} onClick={() => setTab(t.id as Tab)}
                className={'flex items-center gap-2 px-5 py-4 text-xs font-black uppercase tracking-wider transition-all border-b-2 whitespace-nowrap ' +
                  (tab === t.id ? 'border-indigo-600 text-indigo-600 bg-indigo-50/50' : 'border-transparent text-slate-400 hover:text-slate-600')}>
                <i className={'fa-solid ' + t.icon}></i>{t.label}
                {t.id === 'badges' && earnedBadges.length > 0 && (
                  <span className="bg-indigo-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full">{earnedBadges.length}</span>
                )}
              </button>
            ))}
          </div>
              ...
              {trilhas.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {trilhas.map((t, idx) => {
                    ...
                    return (
                      <div key={t.id} className="border border-slate-200 rounded-[14px] p-5 hover:border-indigo-300 transition-all">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400">{done}/{mods} módulos</p>
                            </div>
                          </div>
                          {p === 100
                            ...
                            : <span className="text-[10px] font-black px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-600">{p}%</span>
                          }
                        </div>
                        ...
                        {media !== null && (
                          <p className="text-[10px] text-slate-400 mt-1">Média nos testes: <strong style={{ color: media >= 75 ? '#059669' : '#dc2626' }}>{media}%</strong></p>
                        )}
                      </div>
                    );
                  })}
            ...
            <div className="p-5">
              {myCerts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                  ...
                  <p className="text-xs text-slate-400 mt-1">Conclua uma trilha para emitir seu certificado.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {myCerts.map((c, i) => (
                    <div key={i} className="border-2 border-amber-200 rounded-[14px] p-5 bg-gradient-to-br from-amber-50 to-yellow-50">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <p className="text-sm font-black text-slate-800">{c.trilhaTitulo}</p>
                      ...
                      {c.codigoVerificacao && (
                        <p className="text-[9px] text-slate-400 font-mono bg-white/60 px-2 py-1 rounded-lg">
